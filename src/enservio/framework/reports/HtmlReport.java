...
public class HtmlReport implements ReportType {
	...
	private Boolean isTestLogHeaderTableCreated = Boolean.valueOf(false);
	private Boolean isTestLogMainTableCreated = Boolean.valueOf(false);
	private Boolean isResultSummaryHeaderTableCreated = Boolean.valueOf(false);
	private Boolean isResultSummaryMainTableCreated = Boolean.valueOf(false);
	...
	public HtmlReport(ReportSettings reportSettings, ReportTheme reportTheme) {
		...
		this.testLogPath = (reportSettings.getReportPath()
				+ utilities.getFileSeparator() + "HTML Results"
				+ utilities.getFileSeparator() + reportSettings.getReportName() + ".html");

		this.resultSummaryPath = (reportSettings.getReportPath()
				+ utilities.getFileSeparator() + "HTML Results"
				+ utilities.getFileSeparator() + "Summary" + ".html");
	}
	...
	public void initializeTestLog() {
		...
		try {
			...
			e.printStackTrace();
			throw new setupexceptions("Error while creating HTML test log file");
		}
		...
		try {
			...
			e.printStackTrace();
			throw new setupexceptions("Cannot find HTML test log file");
		}
	...
	private void createTestLogHeaderTable() {
		try {
			...
			String testLogHeaderTable = "\t <body bgcolor='#A9E2F3'> \n\t\t <table id='header'> \n\t\t\t <colgroup> \n\t\t\t\t <col style='width: 25%' /> \n\t\t\t\t <col style='width: 25%' /> \n\t\t\t\t <col style='width: 25%' /> \n\t\t\t\t <col style='width: 25%' /> \n\t\t\t </colgroup> \n\n\t\t\t <thead> \n";
	...
	public void addTestLogHeading(String heading) {
		if (!this.isTestLogHeaderTableCreated.booleanValue()) {
			createTestLogHeaderTable();
			this.isTestLogHeaderTableCreated = Boolean.valueOf(true);
		}
	...
	public void addTestLogTableHeadings() {
		if (!this.isTestLogMainTableCreated.booleanValue()) {
			createTestLogMainTable();
			this.isTestLogMainTableCreated = Boolean.valueOf(true);
		}
		...
		try {
			...
			if ((stepStatus.equals(Status.FAIL))
				...
				if (this.reportSettings.takeScreenshotFailedStep.booleanValue()) {
					testStepRow = testStepRow + "\t\t\t\t\t <td class='"
			...
			testStepRow = testStepRow
					+ "\t\t\t\t\t <td><small>"
					+ utilities.getCurrentFormattedTime(this.reportSettings
			...
			e.printStackTrace();
			throw new setupexceptions("Error while updating HTML test log");
		}
	}
	...
	public void initializeResultSummary() {
		...
		try {
			...
			e.printStackTrace();
			throw new setupexceptions("Cannot find HTML result summary file");
		}
	...
	public void addResultSummaryHeading(String heading) {
		if (!this.isResultSummaryHeaderTableCreated.booleanValue()) {
			createResultSummaryHeaderTable();
			this.isResultSummaryHeaderTableCreated = Boolean.valueOf(true);
		}
	...
	public void addResultSummaryTableHeadings() {
		if (!this.isResultSummaryMainTableCreated.booleanValue()) {
			createResultSummaryMainTable();
			this.isResultSummaryMainTableCreated = Boolean.valueOf(true);
		}
